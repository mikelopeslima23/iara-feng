...
import { getLeads, getCsAtividades, getCsNpsCiclos, getCsNpsRespostasByConta, upsertCsAtividade, supabase } from '../lib/supabase'
...
export default function CS() {
  ...
  useEffect(() => {
    async function load() {
      ...
      setLeads(ls); setAtvs(as); setCiclos(cs)

      // Último relatório trimestral
      const { data: rpts } = await supabase.from('cs_reports').select('id,periodo,tipo,criado_em').order('criado_em', { ascending:false }).limit(1)
      if (rpts && rpts.length > 0) setLastReport(rpts[0])
  ...
  return (
    <div style={{ minHeight:'100vh', background:_D.bg, fontFamily:"'Inter',system-ui,sans-serif" }}>
      ...
      {/* BANNER — Relatório Trimestral */}
      {lastReport && (
        <div style={{ background:'linear-gradient(90deg, #0d0a03, #1a0d06)', borderBottom:`1px solid ${FENG.orange}30`, padding:'12px 20px', display:'flex', alignItems:'center', justifyContent:'space-between', flexWrap:'wrap', gap:10 }}>
          <div style={{ display:'flex', alignItems:'center', gap:10 }}>
            <div style={{ width:32, height:32, background:`${FENG.orange}20`, border:`1px solid ${FENG.orange}40`, borderRadius:7, display:'flex', alignItems:'center', justifyContent:'center', fontSize:16 }}>📋</div>
            <div>
              <div style={{ fontSize:12, fontWeight:700, color:'#f4f0eb' }}>Relatório {lastReport.tipo === 'trimestral' ? 'Trimestral' : 'Mensal'} · {lastReport.periodo}</div>
              <div style={{ fontSize:11, color:'#7a6e80' }}>Gerado em {new Date(lastReport.criado_em).toLocaleDateString('pt-BR')}</div>
            </div>
          </div>
          <button onClick={() => navigate(`/cs/report/${lastReport.id}`)}
            style={{ padding:'7px 16px', background:FENG.orange, color:'white', border:'none', borderRadius:6, fontSize:12, fontWeight:700, cursor:'pointer' }}>
            Ver Relatório →
          </button>
        </div>
      )}
