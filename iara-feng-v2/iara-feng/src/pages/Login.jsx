...
const TEAM = ['Mike Lopes', 'Bruno Braga', 'Jardel Rocha', 'Silvio Vázquez', 'Beni Ertel', 'Alexandre Sivolella']

const CATEGORIAS_RANK = [
  {
    id: 'sem_contato',
    titulo: '👤 Sem contato cadastrado',
    subtitulo: 'Árbitro apitou falta — você nem sabe quem ligar!',
    cor: '#EF4444',
    medalhas: ['🥇 Craque do descuido', '🥈 Reserva do esquecimento', '🥉 No banco mas problemático'],
    motivacao: (n, nome) => n === 0
      ? `✅ ${nome.split(' ')[0]} tá limpo! Goleiro de ferro.`
      : n === 1 ? `${nome.split(' ')[0]} tem ${n} card descoberto. Rápido!`
      : `${nome.split(' ')[0]} tem ${n} cards sem contato. Tá jogando no escuro!`,
  },
  {
    id: 'sem_atividade',
    titulo: '📭 Sem próxima atividade',
    subtitulo: 'Time parado não marca gol. Cadê o próximo passo?',
    cor: '#F59E0B',
    medalhas: ['🥇 Campeão da inércia', '🥈 Vice da paralisia', '🥉 Bronze da procrastinação'],
    motivacao: (n, nome) => n === 0
      ? `✅ ${nome.split(' ')[0]} tem tudo agendado! Craque!`
      : n === 1 ? `${nome.split(' ')[0]} tem ${n} card sem atividade. Agenda!`
      : `${nome.split(' ')[0]} tem ${n} cards parados. O técnico tá olhando...`,
  },
  {
    id: 'atrasadas',
    titulo: '⏰ Atividades atrasadas',
    subtitulo: 'Perdeu o prazo, perdeu o jogo. Olha o cronômetro!',
    cor: '#9D5CF6',
    medalhas: ['🥇 Artilheiro dos atrasos', '🥈 Vice dos calotes', '🥉 Bronze da enrolação'],
    motivacao: (n, nome) => n === 0
      ? `✅ ${nome.split(' ')[0]} em dia! Lateral disciplinado.`
      : n === 1 ? `${nome.split(' ')[0]} tem ${n} atividade vencida.`
      : `${nome.split(' ')[0]} tem ${n} atividades atrasadas. VAR vai pegar!`,
  },
]
...
function RankingModal({ leads, acts, contactsMap, onClose }) {
  ...
  // Calcula pendências por pessoa e categoria
  const stats = TEAM.map(nome => {
    const primeiroNome = nome.split(' ')[0]
    const meusLeads    = leads.filter(l => !l.off && !l.op && l.resp?.includes(primeiroNome))
    ...
    const total = semContato + semAtividade + atrasadas
    return { nome, primeiroNome, semContato, semAtividade, atrasadas, total, cards: meusLeads.length }
  }).filter(s => s.cards > 0) // só quem tem cards

  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.75)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 16, backdropFilter: 'blur(8px)' }}
      onClick={onClose}>
      <div style={{ background: '#13111E', border: '1px solid #2A2640', borderRadius: 20, width: '100%', maxWidth: 580, maxHeight: '88vh', display: 'flex', flexDirection: 'column', boxShadow: '0 32px 100px rgba(0,0,0,.5)' }}
        onClick={e => e.stopPropagation()}>

        {/* Header */}
        <div style={{ padding: '20px 24px 16px', borderBottom: '1px solid #2A2640', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexShrink: 0 }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: 700, color: '#EEEAF8', display: 'flex', alignItems: 'center', gap: 8 }}>
              🏆 Ranking de Pendências
            </div>
            <div style={{ fontSize: 11, color: '#8A84AA', marginTop: 3 }}>Quem tá jogando mal essa semana? Veja o placar do time comercial.</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: '#8A84AA', fontSize: 20, cursor: 'pointer', lineHeight: 1 }}>✕</button>
        </div>

        {/* Body scrollável */}
        <div style={{ overflowY: 'auto', padding: '16px 24px', display: 'flex', flexDirection: 'column', gap: 24 }}>

          {CATEGORIAS_RANK.map(cat => {
            const ranked = [...stats]
              .sort((a, b) => b[cat.id.replace('sem_', 'sem').replace('_', '')] - a[cat.id.replace('sem_', 'sem').replace('_', '')])
            // map category id to stats key
            const key = cat.id === 'sem_contato' ? 'semContato' : cat.id === 'sem_atividade' ? 'semAtividade' : 'atrasadas'
            const rankedFixed = [...stats].sort((a, b) => b[key] - a[key])
            ...
            return (
              <div key={cat.id}>
                ...
                <div style={{ fontSize: 10, color: '#8A84AA', marginBottom: 10, fontStyle: 'italic' }}>{cat.subtitulo}</div>

                {/* Ranking rows */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                  {rankedFixed.map((s, idx) => {
                    const n      = s[key]
                    const isWorst = idx === 0 && n > 0
                    const isClean = n === 0
                    return (
                      <div key={s.nome} style={{
                        display: 'flex', alignItems: 'center', gap: 10,
                        background: isWorst ? `${cat.cor}10` : '#0D0B14',
                        border: `1px solid ${isWorst ? cat.cor + '44' : '#2A2640'}`,
                        borderRadius: 10, padding: '9px 12px',
                        transition: 'all .15s',
                      }}>
                        {/* Posição */}
                        <div style={{ width: 22, flexShrink: 0, textAlign: 'center', fontSize: 14 }}>
                          {isClean ? '✅' : idx === 0 ? '🏴' : idx === 1 ? '🥈' : idx === 2 ? '🥉' : `${idx + 1}.`}
                        </div>

                        {/* Avatar */}
                        <div style={{ width: 28, height: 28, borderRadius: '50%', background: isClean ? '#10B981' : isWorst ? cat.cor : '#3D3860', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 10, fontWeight: 700, color: 'white', flexShrink: 0 }}>
                          {s.nome.split(' ').map(p => p[0]).join('').slice(0, 2)}
                        </div>

                        {/* Nome + mensagem */}
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontSize: 12, fontWeight: 600, color: isClean ? '#6EE7B7' : isWorst ? '#EEEAF8' : '#B8B2D4' }}>
                            {s.primeiroNome}
                            {isWorst && <span style={{ marginLeft: 6, fontSize: 9, background: `${cat.cor}22`, color: cat.cor, borderRadius: 4, padding: '1px 6px', fontWeight: 700 }}>PIOR</span>}
                          </div>
                          <div style={{ fontSize: 10, color: '#8A84AA', marginTop: 1 }}>
                            {cat.motivacao(n, s.nome)}
                          </div>
                        </div>

                        {/* Número */}
                        <div style={{ fontSize: 18, fontWeight: 800, color: isClean ? '#10B981' : n > 5 ? cat.cor : '#B8B2D4', flexShrink: 0, minWidth: 28, textAlign: 'right' }}>
                          {n}
                        </div>
                      </div>
                    )
          ...
          {/* Placar geral */}
          <div style={{ borderTop: '1px solid #2A2640', paddingTop: 16 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: '#C4A7FF', letterSpacing: '.05em', marginBottom: 10 }}>🏆 PLACAR GERAL — TOTAL DE PENDÊNCIAS</div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
              {[...stats].sort((a, b) => b.total - a.total).map((s, idx) => {
                const pct = stats.length > 0 ? Math.max(0, s.total / (Math.max(...stats.map(x => x.total)) || 1) * 100) : 0
                return (
                  <div key={s.nome} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                    <div style={{ fontSize: 11, color: '#8A84AA', width: 16, textAlign: 'right', flexShrink: 0 }}>{idx + 1}</div>
                    <div style={{ fontSize: 11, color: '#B8B2D4', width: 60, flexShrink: 0 }}>{s.primeiroNome}</div>
                    <div style={{ flex: 1, height: 6, background: '#1A1729', borderRadius: 3, overflow: 'hidden' }}>
                      <div style={{ width: `${pct}%`, height: '100%', background: s.total === 0 ? '#10B981' : s.total > 10 ? '#EF4444' : '#F59E0B', borderRadius: 3, transition: 'width .4s ease' }} />
                    </div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: s.total === 0 ? '#10B981' : '#EEEAF8', width: 24, textAlign: 'right', flexShrink: 0 }}>{s.total}</div>
                  </div>
                )
              })}
            </div>
        ...
        {/* Footer */}
        <div style={{ padding: '14px 24px', borderTop: '1px solid #2A2640', display: 'flex', gap: 8, flexShrink: 0 }}>
          <button onClick={onClose}
            style={{ flex: 1, background: '#9D5CF6', border: 'none', borderRadius: 10, color: 'white', padding: '11px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 300, padding: 20, backdropFilter: 'blur(6px)' }}>
      <div style={{ background: '#13111E', border: `1px solid ${alerta.cor}55`, borderRadius: 20, width: '100%', maxWidth: 420, overflow: 'hidden', boxShadow: `0 24px 80px ${alerta.cor}22` }}>
        ...
        <div style={{ height: 3, background: '#2A2640', display: 'flex' }}>
          {alertas.map((_, i) => (
            <div key={i} style={{ flex: 1, background: i <= idx ? alerta.cor : 'transparent', transition: 'background .3s', margin: '0 1px' }} />
          ))}
        </div>
        ...
        <div style={{ background: `${alerta.cor}12`, borderBottom: `1px solid ${alerta.cor}33`, padding: '20px 24px 16px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: '#8A84AA', marginTop: 4 }}>Alerta {idx + 1} de {total}</div>
          </div>
        </div>
        ...
        <div style={{ padding: '16px 24px' }}>
          ...
          <div style={{ marginTop: 14, background: '#0D0B14', border: `1px solid #2A2640`, borderRadius: 10, padding: '10px 14px', maxHeight: 120, overflowY: 'auto' }}>
            ...
            {alerta.leads.length > 8 && (
              <div style={{ fontSize: 11, color: '#8A84AA', marginTop: 4 }}>+{alerta.leads.length - 8} outros</div>
            )}
          </div>
        </div>
        ...
        <div style={{ padding: '0 24px 20px', display: 'flex', gap: 8 }}>
          <button onClick={onClose}
            style={{ flex: 1, background: 'transparent', border: '1px solid #2A2640', borderRadius: 10, color: '#8A84AA', padding: '10px', fontSize: 12, cursor: 'pointer' }}>
            Ver depois
          </button>
          <button onClick={isLast ? onClose : onNext}
            style={{ flex: 2, background: alerta.cor, border: 'none', borderRadius: 10, color: 'white', padding: '11px', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
            {alerta.cta}
            {!isLast && <span style={{ marginLeft: 6, opacity: .8 }}>→</span>}
          </button>
        </div>
      </div>
    </div>
...
export default function Pipeline() {
  ...
  return (
    <div style={{ display: 'flex', height: '100dvh', background: D.bg, fontFamily: "'Inter',system-ui,sans-serif", overflow: 'hidden', position: 'relative' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0, overflow: 'hidden', width: '100%' }}>
        ...
        <div style={{ height: 52, background: D.bg2, borderBottom: `1px solid ${D.border}`, display: 'flex', alignItems: 'center', padding: '0 16px', gap: 10, flexShrink: 0 }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            {/* Pendências do usuário — badge clicável */}
            {(() => {
              ...
              return (
                <button onClick={() => setShowRanking(true)}
                  title="Ver ranking de pendências da equipe"
      ...
      <DailyBriefingModal
        briefing={briefing}
        onNext={() => setBriefing(b => ({ ...b, idx: b.idx + 1 }))}
